...
/* ─── Text Simulator Section ─── */
const TextSimulatorSection = () => {
  const navigate = useNavigate();
  return (
    <section className="py-20 bg-muted/30 relative overflow-hidden">
      <FloatingParticles />
      <div className="max-w-6xl mx-auto px-5 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={fadeRight} className="relative order-2 md:order-1 flex justify-center">
            <motion.div
          ...
          <motion.div variants={fadeLeft} className="order-1 md:order-2">
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
            >
              <span className="text-xs font-bold text-primary tracking-wider">✨ NEW</span>
            </motion.div>
            <h2 className="font-poppins text-3xl sm:text-4xl font-bold text-foreground mb-5 leading-tight">
              Say what you <span className="text-primary">never got to say</span>
            </h2>
            <p className="text-muted-foreground text-lg leading-relaxed mb-6">
              The Text Simulator lets you have the conversation you never had — with an AI version of your ex or situationship, 
              built from their actual patterns. Get real closure without the real damage.
            </p>
            <ul className="space-y-3 mb-8">
              {[
                "AI trained on their specific attachment style & wounds",
                "Psychologically honest — not generic responses",
                "Safe space to process anger, grief, or confusion",
                "12-turn limit keeps it therapeutic, not addictive",
              ].map((t, i) => (
                <motion.li
                  key={t}
                  className="flex items-start gap-3"
