       ...
       <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent value="flags" className="flex-1 overflow-y-auto px-3 py-3 space-y-3 mt-0">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Detected Flags</span>
            <Button
              variant="ghost"
              size="sm"
              ...
              onClick={async () => {
                ...
                try {
                  const { data, error } = await supabase.functions.invoke("detect-flags", {
                    body: { candidateId: id },
                  });
                  if (error) throw error;
                  // Re-fetch candidate to get updated flags
          ...
          {redFlags.length === 0 && greenFlags.length === 0 && (
            <div className="flex flex-col items-center py-12 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground max-w-xs">
                No flags detected yet. Tap Refresh or chat with D.E.V.I. to surface patterns.
              </p>
            </div>
          )}
        </TabsContent>
        ...
        {/* Predictions Tab */}
        <TabsContent value="predictions" className="flex-1 overflow-y-auto px-3 py-3 space-y-4 mt-0">
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 space-y-3">
            <div className="flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-primary" />
              <span className="text-xs font-semibold uppercase tracking-wider">Where This Is Heading</span>
            </div>
            {candidate.ai_description ? (
              <p className="text-sm text-foreground/80 leading-relaxed">{candidate.ai_description}</p>
            ) : (
              <p className="text-sm text-muted-foreground">
                Chat more with D.E.V.I. to unlock AI predictions about {candidate.nickname}.
              </p>
            )}
          </div>
          ...
          {candidate.compatibility_score != null && (
            <div className="rounded-2xl border border-border/50 bg-card p-4 space-y-2">
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Outlook</span>
          ...
          {(candidate.pros as string[] | null)?.length ? (
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-2">
              <span className="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Strengths</span>
              <ul className="space-y-1.5">
                {(candidate.pros as string[]).map((p, i) => (
          ...
          {(candidate.cons as string[] | null)?.length ? (
            <div className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4 space-y-2">
              <span className="text-xs font-semibold text-orange-600 uppercase tracking-wider">Concerns</span>
              <ul className="space-y-1.5">
                {(candidate.cons as string[]).map((c, i) => (
                  <li key={i} className="text-sm flex gap-2">
          ...
          {!candidate.ai_description && !candidate.compatibility_score && (
            <div className="flex flex-col items-center py-12 text-center space-y-3">
              <TrendingUp className="w-8 h-8 text-muted-foreground/50" />
              <p className="text-sm text-muted-foreground max-w-xs">
                Log more interactions and chat with D.E.V.I. to unlock predictions for {candidate.nickname}.
              </p>
            </div>
          )}
        </TabsContent>
