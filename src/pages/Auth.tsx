...
const Auth = () => {
  ...
  // Handle OAuth callback (e.g., Google sign-in redirect)
  useEffect(() => {
    const handleOAuthCallback = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user) {
        // User just signed in via OAuth
        const profile = await checkOnboardingStatus(session.user.id);
        ...
        if (profile?.onboarding_completed) {
          if (!existingPin) {
            ...
            if (session.user.email) {
              localStorage.setItem("datebetter_saved_email", session.user.email);
              localStorage.setItem(getPinEnabledKey(session.user.email), "true");
            }
            navigate("/dashboard");
          }
        } else {
        ...
        <div className="bg-[image:var(--gradient-glass)] backdrop-blur-xl rounded-3xl p-6 shadow-[var(--shadow-elegant)] border border-border/30">
          ...
          {!isForgotPassword && (
            <Button
              type="button"
              variant="outline"
              className="w-full h-12 rounded-xl border-border/50 bg-background/50 hover:bg-background/80 transition-all"
              disabled={loading}
              onClick={async () => {
                setLoading(true);
                try {
